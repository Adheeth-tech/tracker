      ...
      <AppShell>
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center border-b border-gray-150 pb-5">
            <div>
              <h2 className="text-xl font-bold text-slate-800">Trip Dispatch Telemetry</h2>
              <p className="text-xs text-gray-400 font-medium">Real-time GPS log tracking and status audit trail</p>
            </div>
            <Link
              href="/trips"
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="bg-white border border-gray-150 rounded-2xl p-6 shadow-sm space-y-5">
                    <div className="flex justify-between items-center border-b border-gray-100 pb-4">
                      <div>
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Trip Ticket</span>
                        <h3 className="text-md font-bold text-slate-800 font-mono mt-0.5">{trip.trip_code}</h3>
                      </div>
                      <div className="flex items-center gap-2">
                        {!["closed", "cancelled"].includes(trip.status.toLowerCase()) && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 animate-pulse border border-blue-200">
                            Live Polling
                          </span>
                        )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-5 text-xs font-semibold">
                      <div>
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Assigned Driver</span>
                        <p className="font-bold text-slate-800 mt-1">Driver #{trip.driver_id}</p>
                      </div>

                      <div>
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Driver Status</span>
                        <p className={`font-bold mt-1 ${trip.accepted_at ? "text-green-600" : "text-amber-500 italic"}`}>
                          {trip.accepted_at ? "Accepted Offer" : "Awaiting Acceptance"}
                        </p>
                      </div>

                      <div>
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Vehicle Assigned</span>
                        <p className="font-bold text-slate-800 mt-1">Vehicle #{trip.vehicle_id}</p>
                      </div>

                      <div>
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Assigned At</span>
                        <p className="font-mono text-slate-600 mt-1">
                          {formatTimestamp(trip.assigned_at)}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-150 rounded-2xl p-6 shadow-sm space-y-4">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                      ...
                      {trail.length > 0 && (
                        <span className="font-mono text-[10px] text-indigo-650 font-bold bg-indigo-50 px-2 py-0.5 rounded">
                          Last Position: {trail[0].lat.toFixed(4)}, {trail[0].lng.toFixed(4)}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-100 text-xs">
                        ...
                        <tbody className="divide-y divide-gray-50 bg-white">
                          {trail.length === 0 ? (
                            <tr>
                              <td colSpan={5} className="px-4 py-8 text-center text-gray-400 italic font-semibold">
                  ...
                  <div className="bg-white border border-gray-150 rounded-2xl p-5 shadow-sm space-y-4">
                    ...
                    <div className="space-y-3.5 text-xs font-semibold">
                      ...
                      <div className="flex justify-between items-center border-t border-gray-50 pt-2">
                        <span className="text-gray-400">Driver Entered:</span>
                        <span className="font-mono font-bold text-slate-800">
                          {quantity?.driver_entered_litres ? `${quantity.driver_entered_litres.toLocaleString()} L` : "Pending Logs"}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-400">Hotel Confirmed:</span>
                      ...
                      <div className="flex justify-between items-center pt-2.5 border-t border-gray-100">
                        <span className="text-slate-900 font-bold">Final Volume:</span>
                    ...
                    {payment ? (
                      <div className="space-y-3.5 text-xs font-semibold">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-mono font-bold text-slate-800">₹{payment.amount?.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center">
                        ...
                        {payment.transaction_id && (
                          <div className="flex justify-between items-center border-t border-gray-50 pt-2">
                            <span className="text-gray-400 font-semibold">Txn Hash:</span>
                            <span className="font-mono text-[10px] text-slate-650 bg-slate-50 px-1.5 py-0.5 rounded">{payment.transaction_id}</span>
                          </div>
                        )}
                      </div>
                    ) : (
