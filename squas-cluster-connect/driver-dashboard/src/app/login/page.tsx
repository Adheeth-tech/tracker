...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.role !== "driver") {
        setErrorMsg("This portal is reserved for driver logins only.");
      ...
      if (msg.includes("No account for this phone") || msg.includes("404")) {
        setErrorMsg("No driver account found for this phone number. Please register your profile first.");
      } else {
        setErrorMsg(msg || "Invalid OTP code. Please try again.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-md w-full z-10">
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-8 rounded-3xl shadow-2xl relative">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-black tracking-tight text-white">Squas Driver</h1>
            <p className="text-slate-400 text-sm mt-1 text-center font-medium">
              Wastewater Tanker Telematics & Jobs Hub
            </p>
          </div>

          {errorMsg && (
            <div className="bg-red-900/25 border border-red-500/30 text-red-300 p-4 rounded-xl text-xs font-semibold mb-6 flex items-start gap-2 leading-relaxed">
            ...
            <form onSubmit={handleRequestOtp} className="space-y-5">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1.5 leading-normal">
                  Enter your registered driver phone number starting with country code (e.g. +919000000020).
                </p>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-3 px-4 border border-transparent rounded-xl shadow-lg text-sm font-bold text-white bg-indigo-650 hover:bg-indigo-755 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                {loading ? (
          ...
          <div className="mt-6 text-center text-xs font-semibold text-slate-400 border-t border-slate-850 pt-4">
            New tanker driver?{" "}
