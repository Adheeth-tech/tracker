...
import { Droplet, Phone, Lock, ArrowRight, RefreshCw, UserPlus } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.role !== "hotel") {
        setErrorMsg("This portal is for hotel operators only.");
      ...
      if (msg.includes("No account for this phone") || msg.includes("404")) {
        setErrorMsg("Your phone is not registered yet. Please self-register your hotel first.");
      ...
      <div className="max-w-md w-full z-10">
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-8 rounded-3xl shadow-2xl relative">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-black tracking-tight text-white">Squas Hotel</h1>
            <p className="text-slate-400 text-sm mt-1 text-center font-medium">
              Wastewater Pickup & Billing Dashboard
            </p>
          </div>

          {errorMsg && (
            <div className="bg-red-900/25 border border-red-500/30 text-red-300 p-4 rounded-xl text-xs font-semibold mb-6 flex flex-col gap-2">
              ...
              {showRegisterLink && (
                <Link
                  href="/register"
                  className="mt-1 flex items-center gap-1.5 text-indigo-300 hover:text-white font-bold transition-colors text-xs"
                >
                  <UserPlus className="h-4 w-4" />
                  Self-Register Hotel & Operator Account
                </Link>
              )}
            </div>
          )}
            ...
            <form onSubmit={handleRequestOtp} className="space-y-5">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1.5 leading-normal">
                  Enter your registered hotel phone number starting with country code (e.g. +919000000010).
                </p>
              </div>
